...
ridership_month_query = '''
    SELECT *
    FROM CPA_local.ridership_by_month;
    '''

ridership_hour_query = '''
    SELECT *
    FROM CPA_local.ridership_by_hour;
    '''

nationalities_query = '''
    SELECT *
    FROM CPA_local.key_nationalities_riders;
    '''
...
rh_df["Proportion"] = rh_df['Percent'] * 100
...
nationality_fig=px.bar(nationality_df, x='Nationality', y='%', title='Top 5 Riders Nationalities',color_discrete_sequence=['#046845'])
...
revenue_type_dropdown = dcc.Dropdown(
        ...
        value="Total"  # Default to total revenue 
    )
...
def update_monthly_revenue(selected_revenue_type):
   # Query the database based on the selected revenue type
    ...
    monthly_revenue_fig = px.line(revenue_df, x='Month', y=selected_revenue_type, title='Monthly Revenue')
    monthly_revenue_fig.update_layout(common_layout)
    monthly_revenue_fig.update_traces(line=dict(color='#046845'))
